...
{
    public partial class fThickAdd : Form
    {
        public fThickAdd()
        {
            InitializeComponent();
        ...
        private void btnAddThick_Click(object sender, EventArgs e)
        {
            ...
            {
                //ProductCheck
                cmd.CommandText = "select ptName from tPizzaThicks where ptName = @name and ptBitState = 1";
                ...
                {
                    //CreateThick
                    cmd.CommandText = "insert into tPizzaThicks (ptName, ptCost, ptDescription, ptBitState, ptProductType) " +
                    ...
                    cmd.Parameters.AddWithValue("@type", "Pizza Thick");
                    con.Open();
                    cmd.ExecuteNonQuery();
                    con.Close();

                    cmd.CommandText = "select MAX(pizzaThickID) AS ID from tPizzaThicks";
                    con.Open();
                    dr = cmd.ExecuteReader();
                    while (dr.Read())
                    {
                    ...
                    cmd.Parameters.AddWithValue("@typeName", "Pizza Thick");
                    cmd.Parameters.AddWithValue("@name", txtName.Text);
                    cmd.Parameters.AddWithValue("@cost", Convert.ToDouble(txtCost.Text));
                    cmd.Parameters.AddWithValue("@bitState", 1);
                    con.Open();
                    ...
                    if (MessageBox.Show("Ürün eklendi!", "Hamur Ekle", MessageBoxButtons.OK) == DialogResult.OK) this.Close();
                }
