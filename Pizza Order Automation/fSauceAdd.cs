...
{
    public partial class fSauceAdd : Form
    {
        public fSauceAdd()
        {
            InitializeComponent();
        ...
        private void btnAddSauce_Click(object sender, EventArgs e)
        {
            ...
            {
                //ProductCheck
                cmd.CommandText = "select sName from tSauces where sName = @name and sBitState = 1";
                ...
                {
                    //CreateSauce
                    cmd.CommandText = "insert into tSauces (sName, sCost, sDescription, sBitState, sProductType) " +
                    ...
                    cmd.Parameters.AddWithValue("@type", "Sauce");
                    con.Open();
                    cmd.ExecuteNonQuery();
                    con.Close();

                    cmd.CommandText = "select MAX(SauceID) AS ID from tSauces";
                    con.Open();
                    dr = cmd.ExecuteReader();
                    while (dr.Read())
                    {
                    ...
                    cmd.Parameters.AddWithValue("@typeName", "Sauce");
                    cmd.Parameters.AddWithValue("@name", txtName.Text);
                    cmd.Parameters.AddWithValue("@cost", Convert.ToDouble(txtCost.Text));
                    cmd.Parameters.AddWithValue("@bitState", 1);
                    con.Open();
                    ...
                    if (MessageBox.Show("Ürün eklendi!", "Sos Ekle", MessageBoxButtons.OK) == DialogResult.OK) this.Close();
                }
        ...
        private void fSauceAdd_Load(object sender, EventArgs e)
        {
